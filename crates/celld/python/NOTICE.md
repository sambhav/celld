...
- `workers/*.py`: Cloudflare workers-runtime-sdk 1.8.3 sources from
  ...
  Local adaptation: `response.py` uses an optional celld helper for default
  JSON responses and avoids redundant type checks for freshly created native
  responses. JSON serialization and custom response options retain upstream
  behavior. Other worker SDK source files remain unmodified.
